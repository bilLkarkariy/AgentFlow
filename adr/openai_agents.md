Absolument ! Voici un tutoriel complet sur le SDK OpenAI Agents (`openai-agents-python`), basé sur la documentation que vous avez fournie. Ce tutoriel vous guidera à travers l'installation, les concepts clés, la création d'agents, l'orchestration, l'utilisation d'outils, les handoffs, les guardrails, le traçage, et plus encore.
...
1.  Le LLM de l'agent actuel est appelé avec l'historique de la conversation.
2.  Le LLM génère une sortie :
...
3.  La boucle s'arrête aussi si le nombre maximal de tours (`max_turns`) est dépassé (provoquant une exception `MaxTurnsExceeded`).
...
1.  **Instructions de l'Agent :** Via le paramètre `instructions`. Peut être statique ou dynamique (une fonction recevant le `RunContextWrapper`). Idéal pour des informations générales (nom de l'utilisateur, date).
2.  **Input de `Runner.run()` :** Ajoutez des messages système ou utilisateur à la liste d'input initiale.
3.  **Outils Fonctions :** Le LLM peut appeler un outil pour obtenir des informations *à la demande*.
4.  **Retrieval / Recherche Web :** Outils spécifiques (comme `FileSearchTool`, `WebSearchTool`) pour récupérer des informations pertinentes de fichiers ou du web.
...
*   **Visualisation :** Accédez au [Trace viewer dans le tableau de bord OpenAI](https://platform.openai.com/traces) pour voir les exécutions.
*   **Désactivation :**
...
(Nécessite `pip install "openai-agents[viz]"` et l'installation de Graphviz: [https://graphviz.org/download/](https://graphviz.org/download/))
...
Le flux d’exécution d’un flow multi-agents via `agent-runtime` est le suivant :

1. **Définition des outils (Tools)**
   - Chaque `AgentBlock` (ex. `SummarizeBlock`, `TranslateBlock`, etc.) implémente une méthode statique `asTool()` qui retourne :
     ```json
     {
       "name": "SummarizeBlock",
       "description": "Simple summarization assistant",
       "parameters": {
         "type": "object",
     ...
     ```
   - Le service de mapping DSL→Tools collecte toutes ces définitions en un tableau : `tools: OpenAIToolDefinition[]`.

2. **Envoi au side-car Python**
   - Via gRPC (`AgentRuntimeClient.run({ prompt, parameters, tools })`), NestJS envoie :
     - Le prompt utilisateur (ou flux de prompts pour chaque étape).
     - Le tableau `tools` généré.

3. **Runtime Python**
   - Le side-car utilise le SDK [openai-agents](https://github.com/openai/openai-agents) :
     ```python
     from openai_agents import Agent, Tool
     agent = Agent(tools=[Tool.from_dict(t) for t in tools])
     response = agent.run(input_text)
     ```
   - L’Agent décide dynamiquement quel outil appeler.
   - Les appels sont dispatchés aux wrappers Python/JS qui appellent en back-end les fonctions des `AgentBlock`.

4. **Retour des résultats**
   - Chaque outil renvoie un JSON structuré selon son schéma (ex. `{ summary: "..." }`).
   - Le Python side-car renvoie le résultat JSON chunké en streaming gRPC.

5. **Validation et parsing côté NestJS**
   - `AgentRuntimeClient` recueille tous les tokens, reconstruit le JSON complet.
   - Chaque `AgentBlock.parse()` (via Zod) valide et transforme la sortie en objet TypeScript.

6. **Emission des logs et résultat final**
   - NestJS émet les logs temps réel via WebSocket.
   - Le résultat validé est renvoyé à l’API REST.

---

Avec ce workflow, `SummarizeBlock.run(input)` passe bien par le système d’agents : NestJS → Python openai-agents SDK → bloc d’outil → JSON validé → retour.